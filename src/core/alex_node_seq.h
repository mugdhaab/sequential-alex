...
for (int i = 0; i < num_keys; i++) {
    if (position >= data_capacity_) {
        // Handle the case where the data store is full
        break;
    }
...
// Fill remaining slots with the end sentinel
for (int i = position; i < data_capacity_; i++) {
    ALEX_DATA_NODE_KEY_AT(i) = kEndSentinel_;
}
  ...
  std::pair<int, int> insert(const T& key, const P& payload) {
    ...
    key_slots_[num_keys_] = key;
    payload_slots_[num_keys_] = payload;
    num_keys_++;
